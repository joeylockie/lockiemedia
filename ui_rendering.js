...
// --- UI Helper Functions ---
// ... (showMessage, populateDatalist, setSidebarMinimized, hideTooltip remain the same)
// ... (showTooltip needs the FeatureFlagService check added previously)
...
function populateDatalist(datalistElement) {
    if (!datalistElement || typeof uniqueLabels === 'undefined') return; // uniqueLabels from store.js
...
function setSidebarMinimized(minimize) {
    // ... (implementation remains the same, uses global DOM elements)
    ...
    if (minimize) {
        ...
        if (typeof FeatureFlagService !== 'undefined' && FeatureFlagService.isFeatureEnabled('pomodoroTimerHybridFeature') && window.AppFeatures && window.AppFeatures.PomodoroTimerHybrid && typeof window.AppFeatures.PomodoroTimerHybrid.updateSidebarDisplay === 'function') {
...
function showTooltip(element, text) {
    if (typeof FeatureFlagService === 'undefined' || !FeatureFlagService.isFeatureEnabled('tooltipsGuide') ||
        ...
        if (iconTooltip && iconTooltip.style.display === 'block') {
            hideTooltip();
        }
        return;
    }
...
function hideTooltip() {
    ...
    if (typeof tooltipTimeout !== 'undefined' && tooltipTimeout !== null) { // tooltipTimeout from store.js
...
function refreshTaskView() {
    // ... (implementation remains the same, uses ViewManager, FeatureFlagService, and calls other render helpers)
    if (!mainContentArea) {
    ...
    if (FeatureFlagService.isFeatureEnabled('pomodoroTimerHybridFeature') && currentView === 'pomodoro') {
        ...
        if (window.AppFeatures?.CalendarView?.renderFullCalendar) {
            ...
            window.AppFeatures.CalendarView.renderFullCalendar(calendarViewContainer, tasks); // tasks from store.js
        } else {
             ViewManager.setTaskViewMode('list');
             renderTaskListView();
        }
...
// ... (renderTaskListView, renderBulkActionControls, renderTaskDependenciesForViewModal,
//      renderTempSubTasksForAddModal, renderSubTasksForEditModal, renderSubTasksForViewModal,
//      updateSortButtonStates, updateClearCompletedButtonState, updateViewToggleButtonsState,
//      updateYourTasksHeading, styleInitialSmartViewButtons implementations remain the same as the last version,
//      ensuring they use FeatureFlagService, ViewManager, BulkActionService, taskService, utils.js functions,
//      and global state from store.js where appropriate)
// --- (For brevity, the full code of these functions is omitted here but should be the same as your last `ui_rendering_js_refactor_01`) ---
// --- Ensure these functions are present and correctly using the new services/global state as updated in previous steps ---
function renderTaskListView() {
    ...
    if (currentFilterVal === 'inbox') { /* ... */ filteredTasks = tasks.filter(task => !task.completed); }
    else if (currentFilterVal === 'today') { /* ... */ filteredTasks = tasks.filter(task => { if (!task.dueDate || task.completed) return false; const taskDueDate = new Date(task.dueDate + 'T00:00:00'); return taskDueDate.getTime() === today.getTime(); }); }
    else if (currentFilterVal === 'upcoming') { /* ... */ filteredTasks = tasks.filter(task => { if (!task.dueDate || task.completed) return false; const taskDueDate = new Date(task.dueDate + 'T00:00:00'); return taskDueDate.getTime() > today.getTime(); });}
    else if (currentFilterVal === 'completed') { /* ... */ filteredTasks = tasks.filter(task => task.completed); }
    else if (currentFilterVal.startsWith('project_')) { const projectId = parseInt(currentFilterVal.split('_')[1]); if (!isNaN(projectId)) { filteredTasks = tasks.filter(task => task.projectId === projectId && !task.completed); } else { filteredTasks = tasks.filter(task => !task.projectId && !task.completed); }}
    else { filteredTasks = tasks.filter(task => task.label && task.label.toLowerCase() === currentFilterVal.toLowerCase() && !task.completed); }
    if (currentSearchTermVal) { filteredTasks = filteredTasks.filter(task => task.text.toLowerCase().includes(currentSearchTermVal) || (task.label && task.label.toLowerCase().includes(currentSearchTermVal)) || (task.notes && task.notes.toLowerCase().includes(currentSearchTermVal)) || (FeatureFlagService.isFeatureEnabled('projectFeature') && task.projectId && projects.find(p => p.id === task.projectId)?.name.toLowerCase().includes(currentSearchTermVal)));}
    ...
    filteredTasks.forEach((task) => { /* ... create and append task LI elements as before, using getPriorityClass, formatDate, formatTime, FeatureFlagService.isFeatureEnabled, BulkActionService.getSelectedIds etc. ... */ 
        ...
        const hasOpenPrerequisites = FeatureFlagService.isFeatureEnabled('taskDependenciesFeature') && task.dependsOn && task.dependsOn.some(depId => { const dependentTask = tasks.find(t => t.id === depId); return dependentTask && !dependentTask.completed; });
        ...
        if (FeatureFlagService.isFeatureEnabled('projectFeature') && task.projectId && task.projectId !== 0) { const project = projects.find(p => p.id === task.projectId); if (project) { const projSpan = document.createElement('span'); projSpan.className = 'text-purple-600 dark:text-purple-400 flex items-center project-feature-element'; projSpan.innerHTML = `<i class="fas fa-folder mr-1"></i> ${project.name}`; detailsContainer.appendChild(projSpan); }}
        if (task.priority && typeof getPriorityClass === 'function') { const pB = document.createElement('span'); pB.textContent = task.priority; pB.className = `priority-badge ${getPriorityClass(task.priority)}`; detailsContainer.appendChild(pB); }
        ...
        if (FeatureFlagService.isFeatureEnabled('fileAttachments') && task.attachments && task.attachments.length > 0) { const aS = document.createElement('span'); aS.className = 'text-slate-500 dark:text-slate-400 flex items-center file-attachments-element'; aS.innerHTML = `<i class="fas fa-paperclip mr-1"></i> ${task.attachments.length}`; detailsContainer.appendChild(aS); }
        if (FeatureFlagService.isFeatureEnabled('subTasksFeature') && task.subTasks && task.subTasks.length > 0) { const subTaskIcon = document.createElement('span'); subTaskIcon.className = 'text-slate-400 dark:text-slate-500 flex items-center sub-tasks-feature-element'; const completedSubTasks = task.subTasks.filter(st => st.completed).length; subTaskIcon.innerHTML = `<i class="fas fa-tasks mr-1" title="${completedSubTasks}/${task.subTasks.length} sub-tasks completed"></i>`; detailsContainer.appendChild(subTaskIcon); }
        ...
        const editButton = document.createElement('button'); editButton.className = 'task-action-btn text-sky-500 hover:text-sky-700 dark:text-sky-400 dark:hover:text-sky-500'; editButton.innerHTML = '<i class="fas fa-pencil-alt"></i>'; editButton.setAttribute('aria-label', 'Edit task'); editButton.title = 'Edit task'; editButton.addEventListener('click', () => { openViewEditModal(task.id); if (FeatureFlagService.isFeatureEnabled('projectFeature') && window.AppFeatures?.Projects) { window.AppFeatures.Projects.populateProjectDropdowns(); if (modalProjectSelectViewEdit) { modalProjectSelectViewEdit.value = task.projectId || "0"; } } }); actionsDiv.appendChild(editButton);
...
function renderBulkActionControls() { /* ... same as before, uses FeatureFlagService and BulkActionService.getSelectedIds() ... */ 
    if (!bulkActionControlsContainer) { bulkActionControlsContainer = document.getElementById('bulkActionControlsContainer'); if (!bulkActionControlsContainer) return; }
    if (!FeatureFlagService.isFeatureEnabled('bulkActionsFeature')) { bulkActionControlsContainer.classList.add('hidden'); return; }
    const selectedIds = BulkActionService.getSelectedIds(); const hasSelection = selectedIds.length > 0;
    bulkActionControlsContainer.classList.toggle('hidden', !hasSelection); bulkActionControlsContainer.classList.add('bulk-actions-feature-element'); 
    if (hasSelection) { const selectionCountSpan = bulkActionControlsContainer.querySelector('#bulkActionSelectionCount'); if (selectionCountSpan) { selectionCountSpan.textContent = `${selectedIds.length} selected`; } if (bulkCompleteBtn) bulkCompleteBtn.disabled = !hasSelection; if (bulkDeleteBtn) bulkDeleteBtn.disabled = !hasSelection; if (bulkAssignProjectDropdown) bulkAssignProjectDropdown.disabled = !hasSelection; if (bulkChangePriorityDropdown) bulkChangePriorityDropdown.disabled = !hasSelection; if (bulkChangeLabelInput) bulkChangeLabelInput.disabled = !hasSelection;}
    if (selectAllTasksCheckbox) { const currentTaskListEl = taskList || document.getElementById('taskList'); const visibleTasksOnPage = currentTaskListEl ? Array.from(currentTaskListEl.querySelectorAll('.task-item:not(.hidden)')).map(item => parseInt(item.dataset.taskId)) : []; const allVisibleSelected = visibleTasksOnPage.length > 0 && visibleTasksOnPage.every(id => selectedIds.includes(id)); selectAllTasksCheckbox.checked = allVisibleSelected; selectAllTasksCheckbox.indeterminate = !allVisibleSelected && selectedIds.some(id => visibleTasksOnPage.includes(id)); selectAllTasksCheckbox.classList.toggle('hidden', !FeatureFlagService.isFeatureEnabled('bulkActionsFeature') || visibleTasksOnPage.length === 0); }
}
// ... (other render helpers like renderTaskDependenciesForViewModal, renderSubTasksForEditModal, etc. remain the same)
function renderTaskDependenciesForViewModal(task) { if (!FeatureFlagService.isFeatureEnabled('taskDependenciesFeature') || !viewTaskDependsOnList || !viewTaskBlocksTasksList) { if(viewTaskDependenciesSection) viewTaskDependenciesSection.classList.add('hidden'); return; } if(viewTaskDependenciesSection) viewTaskDependenciesSection.classList.remove('hidden'); viewTaskDependsOnList.innerHTML = '';  viewTaskBlocksTasksList.innerHTML = ''; if (task.dependsOn && task.dependsOn.length > 0) { task.dependsOn.forEach(depId => { const dependentTask = tasks.find(t => t.id === depId); const li = document.createElement('li'); li.textContent = dependentTask ? dependentTask.text : `Task ID: ${depId} (Not found)`; if (dependentTask && dependentTask.completed) { li.classList.add('line-through', 'text-slate-400', 'dark:text-slate-500'); } else if (dependentTask) { li.classList.add('text-amber-600', 'dark:text-amber-400');  } viewTaskDependsOnList.appendChild(li); }); } else { viewTaskDependsOnList.innerHTML = '<li class="italic text-slate-500 dark:text-slate-400">None</li>'; } if (task.blocksTasks && task.blocksTasks.length > 0) { task.blocksTasks.forEach(blockedId => { const blockedTask = tasks.find(t => t.id === blockedId); const li = document.createElement('li'); li.textContent = blockedTask ? blockedTask.text : `Task ID: ${blockedId} (Not found)`; viewTaskBlocksTasksList.appendChild(li); }); } else { viewTaskBlocksTasksList.innerHTML = '<li class="italic text-slate-500 dark:text-slate-400">None</li>'; }}
function renderTempSubTasksForAddModal() { if (!FeatureFlagService.isFeatureEnabled('subTasksFeature') || !modalSubTasksListAdd) return; modalSubTasksListAdd.innerHTML = ''; if (tempSubTasksForAddModal.length === 0) { modalSubTasksListAdd.innerHTML = '<li class="text-slate-500 dark:text-slate-400 text-xs text-center py-2">No sub-tasks added yet.</li>'; return; } tempSubTasksForAddModal.forEach((subTask, index) => { const li = document.createElement('li'); li.className = 'flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-700/50 rounded-md text-sm group'; const checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.checked = subTask.completed; checkbox.className = 'form-checkbox h-4 w-4 text-sky-500 rounded border-slate-400 dark:border-slate-500 focus:ring-sky-400 mr-2 cursor-pointer'; checkbox.addEventListener('change', () => { tempSubTasksForAddModal[index].completed = !tempSubTasksForAddModal[index].completed; renderTempSubTasksForAddModal(); }); const textSpan = document.createElement('span'); textSpan.textContent = subTask.text; textSpan.className = `flex-grow break-all ${subTask.completed ? 'line-through text-slate-500 dark:text-slate-400' : 'text-slate-700 dark:text-slate-200'}`; const actionsDiv = document.createElement('div'); actionsDiv.className = 'ml-2 flex items-center opacity-0 group-hover:opacity-100 transition-opacity duration-200'; const deleteBtn = document.createElement('button'); deleteBtn.type = 'button'; deleteBtn.innerHTML = '<i class="fas fa-trash-alt text-xs text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"></i>'; deleteBtn.className = 'p-1'; deleteBtn.title = 'Remove sub-task'; deleteBtn.addEventListener('click', () => { tempSubTasksForAddModal.splice(index, 1); renderTempSubTasksForAddModal(); }); actionsDiv.appendChild(deleteBtn); li.appendChild(checkbox); li.appendChild(textSpan); li.appendChild(actionsDiv); modalSubTasksListAdd.appendChild(li); });}
function renderSubTasksForEditModal(parentId, subTasksListElement) { if (!FeatureFlagService.isFeatureEnabled('subTasksFeature') || !subTasksListElement) return; subTasksListElement.innerHTML = ''; const parentTask = tasks.find(t => t.id === parentId); if (!parentTask || !parentTask.subTasks || parentTask.subTasks.length === 0) { subTasksListElement.innerHTML = '<li class="text-slate-500 dark:text-slate-400 text-xs text-center py-2">No sub-tasks yet. Add one above!</li>'; return; } parentTask.subTasks.forEach(subTask => { const li = document.createElement('li'); li.className = 'flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-700/50 rounded-md text-sm group'; li.dataset.subTaskId = subTask.id; const checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.checked = subTask.completed; checkbox.className = 'form-checkbox h-4 w-4 text-sky-500 rounded border-slate-400 dark:border-slate-500 focus:ring-sky-400 mr-2 cursor-pointer'; checkbox.addEventListener('change', () => { if (window.AppFeatures?.SubTasks?.toggleComplete(parentId, subTask.id)) { renderSubTasksForEditModal(parentId, subTasksListElement); if (currentViewTaskId === parentId && viewTaskDetailsModal && !viewTaskDetailsModal.classList.contains('hidden')) { renderSubTasksForViewModal(parentId, modalSubTasksListViewDetails, viewSubTaskProgress, noSubTasksMessageViewDetails); } showMessage('Sub-task status updated.', 'info'); refreshTaskView(); } }); const textSpan = document.createElement('span'); textSpan.textContent = subTask.text; textSpan.className = `flex-grow break-all ${subTask.completed ? 'line-through text-slate-500 dark:text-slate-400' : 'text-slate-700 dark:text-slate-200'}`; const actionsDiv = document.createElement('div'); actionsDiv.className = 'ml-2 flex items-center opacity-0 group-hover:opacity-100 transition-opacity duration-200'; const editBtn = document.createElement('button'); editBtn.type = 'button'; editBtn.innerHTML = '<i class="fas fa-pencil-alt text-xs text-sky-500 hover:text-sky-700 dark:text-sky-400 dark:hover:text-sky-300"></i>'; editBtn.className = 'p-1'; editBtn.title = 'Edit sub-task'; editBtn.addEventListener('click', () => { const newText = prompt('Edit sub-task:', subTask.text); if (newText !== null && newText.trim() !== '') { if (window.AppFeatures?.SubTasks?.edit(parentId, subTask.id, newText.trim())) { renderSubTasksForEditModal(parentId, subTasksListElement); if (currentViewTaskId === parentId && viewTaskDetailsModal && !viewTaskDetailsModal.classList.contains('hidden')) { renderSubTasksForViewModal(parentId, modalSubTasksListViewDetails, viewSubTaskProgress, noSubTasksMessageViewDetails); } showMessage('Sub-task updated.', 'success'); } else { showMessage('Failed to update sub-task.', 'error'); } } }); const deleteBtn = document.createElement('button'); deleteBtn.type = 'button'; deleteBtn.innerHTML = '<i class="fas fa-trash-alt text-xs text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"></i>'; deleteBtn.className = 'p-1'; deleteBtn.title = 'Delete sub-task'; deleteBtn.addEventListener('click', () => { if (confirm(`Are you sure you want to delete sub-task: "${subTask.text}"?`)) { if (window.AppFeatures?.SubTasks?.delete(parentId, subTask.id)) { renderSubTasksForEditModal(parentId, subTasksListElement); if (currentViewTaskId === parentId && viewTaskDetailsModal && !viewTaskDetailsModal.classList.contains('hidden')) { renderSubTasksForViewModal(parentId, modalSubTasksListViewDetails, viewSubTaskProgress, noSubTasksMessageViewDetails); } showMessage('Sub-task deleted.', 'success'); refreshTaskView(); } else { showMessage('Failed to delete sub-task.', 'error'); } } }); actionsDiv.appendChild(editBtn); actionsDiv.appendChild(deleteBtn); li.appendChild(checkbox); li.appendChild(textSpan); li.appendChild(actionsDiv); subTasksListElement.appendChild(li); });}
function renderSubTasksForViewModal(parentId, subTasksListElement, progressElement, noSubTasksMessageElement) { if (!FeatureFlagService.isFeatureEnabled('subTasksFeature') || !subTasksListElement || !progressElement || !noSubTasksMessageElement) return; subTasksListElement.innerHTML = ''; const parentTask = tasks.find(t => t.id === parentId); if (!parentTask || !parentTask.subTasks || parentTask.subTasks.length === 0) { progressElement.textContent = ''; noSubTasksMessageElement.classList.remove('hidden'); subTasksListElement.classList.add('hidden'); return; } noSubTasksMessageElement.classList.add('hidden'); subTasksListElement.classList.remove('hidden'); let completedCount = 0; parentTask.subTasks.forEach(subTask => { if (subTask.completed) completedCount++; const li = document.createElement('li'); li.className = 'flex items-center text-sm group py-1'; li.dataset.subTaskId = subTask.id; const checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.checked = subTask.completed; checkbox.className = 'form-checkbox h-4 w-4 text-sky-500 rounded border-slate-400 dark:border-slate-500 focus:ring-sky-400 mr-2 cursor-pointer flex-shrink-0'; checkbox.addEventListener('change', () => { if (window.AppFeatures?.SubTasks?.toggleComplete(parentId, subTask.id)) { renderSubTasksForViewModal(parentId, subTasksListElement, progressElement, noSubTasksMessageElement); if (editingTaskId === parentId && viewEditTaskModal && !viewEditTaskModal.classList.contains('hidden')) { renderSubTasksForEditModal(parentId, modalSubTasksListViewEdit); } showMessage('Sub-task status updated.', 'info'); refreshTaskView(); } }); const textSpan = document.createElement('span'); textSpan.textContent = subTask.text; textSpan.className = `flex-grow break-all ${subTask.completed ? 'line-through text-slate-500 dark:text-slate-400' : 'text-slate-700 dark:text-slate-300'}`; li.appendChild(checkbox); li.appendChild(textSpan); subTasksListElement.appendChild(li); }); progressElement.textContent = `${completedCount}/${parentTask.subTasks.length} completed`;}
function updateSortButtonStates() { const currentSortVal = ViewManager.getCurrentSort(); [sortByDueDateBtn, sortByPriorityBtn, sortByLabelBtn].forEach(btn => { if (btn) { let sortType = ''; if (btn === sortByDueDateBtn) sortType = 'dueDate'; else if (btn === sortByPriorityBtn) sortType = 'priority'; else if (btn === sortByLabelBtn) sortType = 'label'; btn.classList.toggle('sort-btn-active', currentSortVal === sortType); } });}
function updateClearCompletedButtonState() { if (!settingsClearCompletedBtn) return; const hasCompleted = tasks.some(task => task.completed); settingsClearCompletedBtn.disabled = !hasCompleted; settingsClearCompletedBtn.classList.toggle('opacity-50', !hasCompleted); settingsClearCompletedBtn.classList.toggle('cursor-not-allowed', !hasCompleted); const activeClasses = ['bg-red-50', 'hover:bg-red-100', 'text-red-700', 'dark:bg-red-900/50', 'dark:hover:bg-red-800/70', 'dark:text-red-300']; const disabledClasses = ['bg-slate-100', 'text-slate-400', 'dark:bg-slate-700', 'dark:text-slate-500', 'hover:bg-slate-100', 'dark:hover:bg-slate-700']; if (hasCompleted) { settingsClearCompletedBtn.classList.remove(...disabledClasses); settingsClearCompletedBtn.classList.add(...activeClasses); } else { settingsClearCompletedBtn.classList.remove(...activeClasses); settingsClearCompletedBtn.classList.add(...disabledClasses); }}
function updateViewToggleButtonsState() { const currentView = ViewManager.getCurrentTaskViewMode(); const isKanbanEnabled = FeatureFlagService.isFeatureEnabled('kanbanBoardFeature'); const isCalendarEnabled = FeatureFlagService.isFeatureEnabled('calendarViewFeature'); const isPomodoroEnabled = FeatureFlagService.isFeatureEnabled('pomodoroTimerHybridFeature'); const isKanbanActive = isKanbanEnabled && currentView === 'kanban'; const isCalendarActive = isCalendarEnabled && currentView === 'calendar'; const isPomodoroActive = isPomodoroEnabled && currentView === 'pomodoro'; if (kanbanViewToggleBtn && kanbanViewToggleBtnText) { kanbanViewToggleBtn.classList.toggle('hidden', !isKanbanEnabled); if (isKanbanEnabled) { const icon = kanbanViewToggleBtn.querySelector('i'); kanbanViewToggleBtnText.textContent = isKanbanActive ? 'List' : 'Board'; kanbanViewToggleBtn.title = isKanbanActive ? 'Switch to List View' : 'Switch to Board View'; if (icon) { icon.classList.toggle('fa-columns', !isKanbanActive); icon.classList.toggle('fa-list-ul', isKanbanActive); } kanbanViewToggleBtn.classList.toggle('bg-purple-500', isKanbanActive); kanbanViewToggleBtn.classList.toggle('text-white', isKanbanActive); kanbanViewToggleBtn.classList.toggle('dark:bg-purple-600', isKanbanActive); } } if (calendarViewToggleBtn && calendarViewToggleBtnText) { calendarViewToggleBtn.classList.toggle('hidden', !isCalendarEnabled); if (isCalendarEnabled) { const icon = calendarViewToggleBtn.querySelector('i'); calendarViewToggleBtnText.textContent = isCalendarActive ? 'List' : 'Calendar'; calendarViewToggleBtn.title = isCalendarActive ? 'Switch to List View' : 'Switch to Calendar View'; if (icon) { icon.classList.toggle('fa-calendar-week', !isCalendarActive); icon.classList.toggle('fa-list-ul', isCalendarActive); } calendarViewToggleBtn.classList.toggle('bg-teal-500', isCalendarActive); calendarViewToggleBtn.classList.toggle('text-white', isCalendarActive); calendarViewToggleBtn.classList.toggle('dark:bg-teal-600', isCalendarActive); } }  if (pomodoroViewToggleBtn && pomodoroViewToggleBtnText) { pomodoroViewToggleBtn.classList.toggle('hidden', !isPomodoroEnabled); if (isPomodoroEnabled) { const icon = pomodoroViewToggleBtn.querySelector('i'); pomodoroViewToggleBtnText.textContent = isPomodoroActive ? 'List' : 'Pomodoro'; pomodoroViewToggleBtn.title = isPomodoroActive ? 'Switch to List View' : 'Switch to Pomodoro View'; if (icon) { icon.classList.toggle('fa-stopwatch', !isPomodoroActive); icon.classList.toggle('fa-list-ul', isPomodoroActive); } pomodoroViewToggleBtn.classList.toggle('bg-rose-500', isPomodoroActive); pomodoroViewToggleBtn.classList.toggle('text-white', isPomodoroActive); pomodoroViewToggleBtn.classList.toggle('dark:bg-rose-600', isPomodoroActive); } } const sortButtonsVisible = currentView === 'list'; if (sortByDueDateBtn) sortByDueDateBtn.classList.toggle('hidden', !sortButtonsVisible); if (sortByPriorityBtn) sortByPriorityBtn.classList.toggle('hidden', !sortButtonsVisible); if (sortByLabelBtn) sortByLabelBtn.classList.toggle('hidden', !sortButtonsVisible); }
function updateYourTasksHeading() { if (!yourTasksHeading) return; const currentView = ViewManager.getCurrentTaskViewMode(); const currentFilterVal = ViewManager.getCurrentFilter(); if (FeatureFlagService.isFeatureEnabled('pomodoroTimerHybridFeature') && currentView === 'pomodoro') { yourTasksHeading.textContent = 'Pomodoro Timer'; } else if (FeatureFlagService.isFeatureEnabled('calendarViewFeature') && currentView === 'calendar') { yourTasksHeading.textContent = 'Calendar'; } else if (FeatureFlagService.isFeatureEnabled('kanbanBoardFeature') && currentView === 'kanban') { yourTasksHeading.textContent = 'Kanban Board'; } else if (currentFilterVal.startsWith('project_')) { const projectId = parseInt(currentFilterVal.split('_')[1]); const project = projects.find(p => p.id === projectId); yourTasksHeading.textContent = project ? `Project: ${project.name}` : 'Unknown Project Tasks'; } else { const filterText = currentFilterVal.charAt(0).toUpperCase() + currentFilterVal.slice(1); yourTasksHeading.textContent = `${filterText} Tasks`; }}
function styleInitialSmartViewButtons() { const currentFilterVal = ViewManager.getCurrentFilter(); if (smartViewButtons && smartViewButtons.length > 0) { smartViewButtons.forEach(button => { button.classList.add('bg-slate-200', 'text-slate-700', 'hover:bg-slate-300', 'dark:bg-slate-700', 'dark:text-slate-300', 'dark:hover:bg-slate-600'); button.querySelector('i')?.classList.add('text-slate-500', 'dark:text-slate-400'); }); const initialActiveButton = Array.from(smartViewButtons).find(btn => btn.dataset.filter === currentFilterVal); if (initialActiveButton) { initialActiveButton.classList.remove('bg-slate-200', 'text-slate-700', 'hover:bg-slate-300', 'dark:bg-slate-700', 'dark:text-slate-300', 'dark:hover:bg-slate-600'); initialActiveButton.querySelector('i')?.classList.remove('text-slate-500', 'dark:text-slate-400'); initialActiveButton.classList.add('bg-sky-500', 'text-white', 'font-semibold', 'dark:bg-sky-600', 'dark:text-sky-50'); initialActiveButton.querySelector('i')?.classList.add('text-sky-100', 'dark:text-sky-200'); } } }


/**
 * NEW: Initializes subscriptions to events from the EventBus.
 * This function should be called once, e.g., from main.js after DOM is ready.
 */
function initializeUiRenderingSubscriptions() {
    ...
    EventBus.subscribe('tasksChanged', (updatedTasks) => {
        console.log("[UI Rendering] Event received: tasksChanged. Refreshing view.");
        // tasks global variable in store.js is already updated by the publisher (e.g., saveTasks)
        refreshTaskView();
        updateClearCompletedButtonState(); // Also depends on task completion status
    });

    EventBus.subscribe('projectsChanged', (updatedProjects) => {
        console.log("[UI Rendering] Event received: projectsChanged. Refreshing view (if project names affect task display).");
        // projects global variable in store.js is updated by the publisher
        refreshTaskView(); // If project names are shown in the task list or affect filtering
        // If project filters in sidebar need updating, that's handled by 'uniqueProjectsChanged'
    });
    
    EventBus.subscribe('uniqueProjectsChanged', (newUniqueProjects) => {
        console.log("[UI Rendering] Event received: uniqueProjectsChanged. Repopulating project UI elements.");
        // uniqueProjects global variable in store.js is updated
        if (typeof window.AppFeatures?.Projects?.populateProjectFilterList === 'function') {
            window.AppFeatures.Projects.populateProjectFilterList();
        }
        if (typeof window.AppFeatures?.Projects?.populateProjectDropdowns === 'function') {
            window.AppFeatures.Projects.populateProjectDropdowns();
        }
    });

    EventBus.subscribe('kanbanColumnsChanged', (updatedColumns) => {
        console.log("[UI Rendering] Event received: kanbanColumnsChanged.");
        // kanbanColumns global variable in store.js is updated
        if (ViewManager.getCurrentTaskViewMode() === 'kanban' && FeatureFlagService.isFeatureEnabled('kanbanBoardFeature')) {
            refreshTaskView(); // This will call the Kanban board's render function
        }
    });

    EventBus.subscribe('filterChanged', (eventData) => {
        console.log("[UI Rendering] Event received: filterChanged. Refreshing view and heading.");
        // currentFilter and currentSort in store.js are updated by ViewManager
        refreshTaskView();
        updateYourTasksHeading(); // Update heading based on new filter
        updateSortButtonStates(); // Reset/update sort button active state
        // Smart view button active states are handled by setFilter in ui_event_handlers.js
    });

    EventBus.subscribe('sortChanged', (newSort) => {
        console.log("[UI Rendering] Event received: sortChanged. Refreshing view and sort buttons.");
        // currentSort in store.js is updated by ViewManager
        refreshTaskView();
        updateSortButtonStates();
    });

    EventBus.subscribe('searchTermChanged', (newSearchTerm) => {
        console.log("[UI Rendering] Event received: searchTermChanged. Refreshing view.");
        // currentSearchTerm in store.js is updated by ViewManager
        refreshTaskView();
    });

    EventBus.subscribe('viewModeChanged', (newViewMode) => {
        console.log("[UI Rendering] Event received: viewModeChanged. Refreshing view and UI states.");
        // currentTaskViewMode in store.js is updated by ViewManager
        refreshTaskView(); // This will switch to the correct view rendering function
        updateViewToggleButtonsState();
        updateYourTasksHeading();
    });

    // 'featureFlagsUpdated' event is handled by applyActiveFeatures in ui_event_handlers.js,
    // which in turn calls refreshTaskView. So, no direct subscription here for refreshTaskView.
    // However, specific UI elements might need to react directly.
    EventBus.subscribe('featureFlagsUpdated', (updateData) => {
        console.log("[UI Rendering] Event received: featureFlagsUpdated. Certain UI states might need refresh.");
        // Example: if a specific component needs to re-evaluate its display based on a flag,
        // not covered by applyActiveFeatures's general class toggling.
        // For now, applyActiveFeatures handles most of this.
        updateViewToggleButtonsState(); // Re-check visibility of view toggle buttons
        // refreshTaskView(); // This is already called by applyActiveFeatures
    });


    EventBus.subscribe('labelsChanged', (newLabels) => {
        console.log("[UI Rendering] Event received: labelsChanged. Populating datalists.");
        // uniqueLabels in store.js is updated
        ...
        // If manage labels modal is open, its list should also refresh.
        if (manageLabelsModal && !manageLabelsModal.classList.contains('hidden') && typeof populateManageLabelsList === 'function') {
            populateManageLabelsList();
        }
    });

    EventBus.subscribe('bulkSelectionChanged', (selectedIds) => {
        console.log("[UI Rendering] Event received: bulkSelectionChanged. Rendering bulk action controls.");
        // selectedTaskIdsForBulkAction in store.js is updated by BulkActionService
        renderBulkActionControls();
    });
...
// console.log("ui_rendering.js parsed.");
