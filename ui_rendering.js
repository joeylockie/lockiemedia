...
let smartViewButtonsContainer, smartViewButtons, messageBox; // messageBox is key here
...
// MODIFIED: Renamed to _displayMessage and no longer exported
        ...
        checkbox.onchange = () => { 
            if (SubTasksFeature && SubTasksFeature.toggleComplete) { 
                if (SubTasksFeature.toggleComplete(actualParentId, st.id)) { 
                    ...
                    _displayMessage('Failed to toggle sub-task.', 'error');  // Use internal _displayMessage
                }
            } else { 
            ...
            if (newText !== null && newText.trim() !== '') { 
                if (SubTasksFeature && SubTasksFeature.edit) { 
                    if (SubTasksFeature.edit(actualParentId, st.id, newText.trim())) { 
                        ...
                        _displayMessage('Failed to edit sub-task.', 'error'); // Use internal _displayMessage
                    }
                } else { 
            ...
            if (confirm('Delete this sub-task?')) { 
                if (SubTasksFeature && SubTasksFeature.delete) { 
                    if (SubTasksFeature.delete(actualParentId, st.id)) { 
                        ...
                        _displayMessage('Failed to delete sub-task.', 'error'); // Use internal _displayMessage
                    }
                } else { 
...
export function initializeUiRenderingSubscriptions() { 
    ...
    // MODIFIED: Add subscription for 'displayUserMessage'
