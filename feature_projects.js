...
import ViewManager from './viewManager.js'; // Imported to get current filter for deleteProjectById logic
...
function handleAddNewProject(event) {
    ...
    if (newProjectAdded) { //
        ...
        // Logging for successful add is handled by projectService.addProject
    }
    // Logging for empty name or duplicate is handled by projectService.addProject
}
...
function handleEditProject(projectId) {
    ...
    if (!project || project.id === 0) { //
        // User message is handled by projectService or this would be a redundant log if projectService already logs it.
        // For now, assume projectService handles the user message and logs the specific reason.
        ...
        // showMessage('Project not found or "No Project" cannot be edited.', 'error'); // Covered by projectService now
        return; //
    }
    ...
    if (updatedProject) { //
        ...
        // Logging for successful update handled by projectService.updateProjectName
    }
    // Logging for failed update (empty name, duplicate) handled by projectService.updateProjectName
}
...
function handleDeleteProject(projectId) {
    ...
    if (!project || project.id === 0) { //
        ...
        // showMessage('Project not found or "No Project" cannot be deleted.', 'error'); // Covered by projectService now
        return; //
    }
    ...
    if (deleteProjectById(projectId)) { //
        ...
        // Logging for successful deletion handled by projectService.deleteProjectById
        ...
        // Logging for failed deletion handled by projectService.deleteProjectById
        // showMessage('Failed to delete project.', 'error'); // Covered by projectService
...
function populateProjectFilterList() {
    ...
    // Active state styling is handled by ui_rendering.js via the 'filterChanged' event triggered by setFilter.
}
