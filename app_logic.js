...
async function loadFeatureFlags() {
    try {
        ...
        if (!response.ok) {
            ...
            // Ensure all feature flags have a default value if not fetched
            Object.keys(featureFlags).forEach(flag => {
                if (typeof featureFlags[flag] === 'undefined') featureFlags[flag] = false;
            });
            return;
        }
...
// --- Sub-task Logic ---
// ... (Sub-task logic functions remain unchanged for this step) ...
function addSubTaskLogic(parentId, subTaskText) {
    ...
    tasks[parentTaskIndex].subTasks.push(newSubTask); saveTasks(); return true;
}
function editSubTaskLogic(parentId, subTaskId, newText) {
    ...
    tasks[parentTaskIndex].subTasks[subTaskIndex].text = newText.trim(); saveTasks(); return true;
}
function toggleSubTaskCompleteLogic(parentId, subTaskId) {
    ...
    const subTaskIndex = tasks[parentTaskIndex].subTasks.findIndex(st => st.id === subTaskId);
    if (subTaskIndex === -1) return false;
    tasks[parentTaskIndex].subTasks[subTaskIndex].completed = !tasks[parentTaskIndex].subTasks[subTaskIndex].completed; saveTasks(); return true;
}
function deleteSubTaskLogic(parentId, subTaskId) {
    if (!featureFlags.subTasksFeature || !parentId || !subTaskId) return false;
    const parentTaskIndex = tasks.findIndex(t => t.id === parentId);
    if (parentTaskIndex === -1) return false;
    tasks[parentTaskIndex].subTasks = tasks[parentTaskIndex].subTasks.filter(st => st.id !== subTaskId); saveTasks(); return true;
}

// --- Timer Logic ---
// ... (Timer logic functions remain unchanged for this step) ...
function updateLiveTimerDisplay(taskId) { 
    if (!featureFlags.taskTimerSystem) return;
    const task = tasks.find(t => t.id === taskId);
    if (!task || !task.timerIsRunning || task.completed) { if (currentTaskTimerInterval) clearInterval(currentTaskTimerInterval); currentTaskTimerInterval = null; return; }
}
function startTimerLogic(taskId) {
    if (!featureFlags.taskTimerSystem || !taskId) return false;
    const taskIndex = tasks.findIndex(t => t.id === taskId); if (taskIndex === -1) return false;
    const task = tasks[taskIndex];
    if (task.completed || (task.actualDurationMs && task.actualDurationMs > 0 && !task.timerIsPaused && !task.timerIsRunning)) return false;
    task.timerIsRunning = true; task.timerIsPaused = false; task.timerStartTime = Date.now(); saveTasks(); return true;
}
function pauseTimerLogic(taskId) {
    if (!featureFlags.taskTimerSystem || !taskId) return false;
    const taskIndex = tasks.findIndex(t => t.id === taskId); if (taskIndex === -1 || !tasks[taskIndex].timerIsRunning) return false;
    const task = tasks[taskIndex];
    const elapsed = Date.now() - (task.timerStartTime || Date.now());
    task.timerAccumulatedTime = (task.timerAccumulatedTime || 0) + elapsed;
    task.timerIsRunning = false; task.timerIsPaused = true; task.timerStartTime = null; saveTasks(); return true;
}
function stopTimerLogic(taskId) {
    if (!featureFlags.taskTimerSystem || !taskId) return false;
    const taskIndex = tasks.findIndex(t => t.id === taskId); if (taskIndex === -1) return false;
    const task = tasks[taskIndex];
    if (task.timerIsRunning) { const elapsed = Date.now() - (task.timerStartTime || Date.now()); task.timerAccumulatedTime = (task.timerAccumulatedTime || 0) + elapsed; }
    if (task.timerAccumulatedTime > 0) { task.actualDurationMs = task.timerAccumulatedTime; }
    task.timerIsRunning = false; task.timerIsPaused = false; task.timerStartTime = null; saveTasks(); return true;
}
