...
import { applyFuzziness } from './fuzzyLogicService.js'; // Import from the new service

// --- Recurrence Logic ---

/**
 * Creates the next task in a recurring series based on a completed task.
 * @param {object} completedTask - The task object that was just completed.
 */
function createNextRecurringTask(completedTask) {
    ...
    if (rule.daysOfWeek && rule.daysOfWeek.length > 0) {
        rruleOptions.byweekday = rule.daysOfWeek.map(day => RRule[day]);
    }
    ...
    LoggingService.info(`[${functionName}] Creating next recurring task.`, {
        ...
        newDueDate: newTaskData.dueDate,
